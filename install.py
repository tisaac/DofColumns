...
class Installer():
  ...
  def setup(self,args):
    parser = argparse.ArgumentParser(description='Install DofColumns')
    parser.add_argument('--src-dir', help='Location of DofColumns source', default=os.environ.get('DOFCOLUMNS_DIR',os.getcwd()))
  ...
  def setupDirectories(self,args):
    ...
    self.destConfDir       = os.path.join(self.destLibDir, 'dofcolumns-conf')
    return
  ...
  def checkDestdir(self):
    if os.path.exists(self.destDir):
      if os.path.samefile(self.destDir, self.rootDir):
        print '********************************************************************'
        print 'Incorrect prefix usage. Specified prefix same as DofColumns source directory'
        print '********************************************************************'
        sys.exit(1)
      if os.path.samefile(self.destDir, os.path.join(self.rootDir,self.arch)):
        print '********************************************************************'
        print 'Incorrect prefix usage. Specified destDir same as DofColumns build directory'
        print '********************************************************************'
        sys.exit(1)
    return
  ...
  def createUninstaller(self):
    uninstallscript = os.path.join(self.destConfDir, 'uninstall_DofColumns.py')
...
''')
  ...
  def installIncludes(self):
    self.copies.extend(self.copyfile(os.path.join(self.rootDir,'DofColumns.h'),os.path.join(self.destIncludeDir,'DofColumns.h')))
    return
...
make DOFCOLUMNS_DIR=%s PETSC_ARCH="" test
====================================\
''' % (self.destDir,self.destDir)
  ...
  def runcopy(self):
    print '*** Installing DofColumns at location:',self.destDir, ' ***'
